...
<?php include_partial('bon/assets') ?>
<?php use_stylesheet('print.css', '', array('media' => 'print')) ?>
  ...
  <div class="productName">
    <span>Rapport de stats Factures<?php if($hasDate): ?> du <?php echo format_date($from) ?> au <?php echo format_date($to) ?><?php endif; ?></span>
    <div class="actions">
		<a href="<?php echo url_for('@bon') ?>">Retour à la liste</a>
  		...
  		<tbody>
			...
			<tr class="noThPage">
  			...
  			<tr>
  				...
  				<td><?php echo $bon->getQualite() ?></td>
  				<td><?php echo $bon->getMetrage() ?></td>
  			...
  			<?php 
  				if ($bon->getType() == Bon::TYPE_FACTURE) {
  					...
  					$total_metrage += round($bon->getMetrage(), 2);
					$total_metrage_all += round($bon->getMetrage(), 2);
  			...
  			<tr class="statTotalMax">
  				...
  				<td><strong><?php echo number_format(round($total_metrage_all, 2), 2, '.', ' '); ?></strong></td>
