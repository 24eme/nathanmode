...
class Activite
{
	...
	public function getMontant($devise = 1, $client = null, $fournisseur = null)
	{
		...
		$reqCredit = "SELECT SUM(b.montant) as montant FROM bon b WHERE b.type != 'Facture' AND b.date <= '".$this->to."' AND b.date >= '".$this->from."' AND b.devise_montant_id = ".$devise." AND b.montant > 0".$where;
	...
	public function getDetails($devise = 1, $client = null, $fournisseur = null)
	{
		...
		$reqCredit = "SELECT -1 as coef, s.libelle, b.qualite, sum(b.metrage) as metrage, sum(b.piece) as piece, sum(b.montant) as montant FROM bon b INNER JOIN saison s ON b.saison_id = s.id WHERE b.type != 'Facture' AND b.date <= '".$this->to."' AND b.date >= '".$this->from."' AND b.devise_montant_id = ".$devise." AND b.montant > 0".$where." GROUP BY s.libelle, b.qualite";
	...
	public function getMontantByClient()
	{
		...
		$reqCredit = "SELECT c.id as id, c.raison_sociale as client, SUM(b.montant) as montant FROM bon b JOIN client c ON b.client_id = c.id WHERE b.type != 'Facture' AND b.date <= '".$this->to."' AND b.date >= '".$this->from."' AND b.devise_montant_id = 1 AND b.montant > 0 GROUP BY b.client_id";
